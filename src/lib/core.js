...
function calculate(grossInput, allowancesInput, taxReliefInput, isAnnual = false) {
    const grossIncome = isAnnual ? new Decimal(grossInput).dividedBy(NUM_OF_MONTHS) : new Decimal(grossInput);
    const allowances = isAnnual ? new Decimal(allowancesInput).dividedBy(NUM_OF_MONTHS) : new Decimal(allowancesInput);
    const taxRelief = isAnnual ? new Decimal(taxReliefInput).dividedBy(NUM_OF_MONTHS) : new Decimal(taxReliefInput);

    return computeTaxes({grossIncome, allowances, taxRelief, taxRates: monthlyTaxRates});
}
